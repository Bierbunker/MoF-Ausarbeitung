...
\end{align}
Die Schwingungsfrequenzen in einem Rotations-Schwingungsspektrum geben Informationen über die Energieniveaus der Molekülschwingungen. 
...
\label{q:25}
Das Frank-Gordon Prinzip besagt, dass elektronische Übergänge so schnell stattfinden 10$^{-15}$ s , dass der Kernabstand sich effektiv nicht ändert.
...
Wenn ein Molekül nun von einem elektronischen Zustand in einen anderen übergeht, so ist dieser Übergang umso wahrscheinlicher, je mehr die Vibrations-Wellenfunktionen der beiden Zustände zueinander kompatibel sind (z. B. beim gleichen Kernabstand ein Maximum haben) . 
siehe Abbildung \ref{fig:gordon}.
Am Beispiel der Zustände in Abbildung \ref{fig:gordon} bedeutet dies: Vom Vibrations-Grundzustand ( $\nu'' = 0$) im elektronischen Grundzustand ist der wahrscheinlichste Übergang in den elektronisch angeregten Zustand derjenige, der im Vibrations-Zustand $\nu' = 2 $ endet. Übergänge in andere Vibrations-Zustände können auch stattfinden, allerdings ist die Wahrscheinlichkeit dafür geringer.
...
Um so einen Übergang zu modelieren  wird ein Dipoloperator eingeführt:
\begin{equation}
